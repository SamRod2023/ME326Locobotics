...
class LocobotExample(object):
    ...
    def __init__(self):
        ...
        self.move_complete = rospy.Publisher("/locobot/mobile_base/move_complete", String, queue_size=2)
        self.move_complete_pick = rospy.Publisher("/locobot/mobile_base/move_complete_pick", String, queue_size=2)

        self.L = 0.4 #this is the distance of the point P (x,y) that will be controlled for position. The locobot base_link frame points forward in the positive x direction, the point P will be on the positive x-axis in the body-fixed frame of the robot mobile base
    ...
    def set_target_pose_origin(self, data):
        self.target_pose.position.x = 0
    ...
    def set_target_pose_block(self, data):
        if self.set_target == False:
            self.set_target = True
            self.target_pose.position.x = data.pose.position.z
            self.target_pose.position.y = -1*data.pose.position.x
    ...
    def mobile_base_callback(self, data):
        ...
        Kp_mat = 0.7*np.eye(2) #proportional gain matrix, diagonal with gain of 0.2 (for PID control)
        ...
        if err_magnitude < self.goal_reached_error:
            ...
            if self.state == "holding":
                self.move_complete.publish("done")
                self.state = "no mas"
            elif self.state == "something":
                print(self.state)
                self.move_complete_pick.publish("done")
                self.state = "no mas"
    ...
    def go_to_pose(self,target_pose=None):
        ...
        rospy.Subscriber("/locobot/camera_cube_locator", Marker, self.set_target_pose_block)
        rospy.Subscriber("/locobot/mobile_base/grasp_complete", String, self.set_target_pose_origin)
